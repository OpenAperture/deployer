defmodule OpenAperture.Deployer.GitHub do
  @moduledoc "Interacts with GitHub."

  require Logger
  alias   OpenAperture.Deployer

  @doc """
  Returns the project name of a repository.
  ...
  def get_project_name(src_repo) do
    URI.parse(src_repo).path |> get_after_last_forward_slash
  end
  ...
  String
  """
  @spec resolve_repo_url(String.t()) :: String.t()
  def resolve_repo_url(github_repo) do
    "https://#{System.get_env("GITHUB_OAUTH_TOKEN")}:x-oauth-basic@github.com/#{github_repo}.git"
  end
  ...
  @spec create!(Map) :: pid
  def create!(github_options) do
    case Github.create(github_options) do
      {:ok, github} -> github
      {:error, reason} -> raise "Failed to create Github Agent:  #{reason}"
    end
  end
  ...
  def clone(github) do
    ...
    if (github_options == nil) do
      ...
      Logger.info("Cloning #{github_options[:repo_url]} into directory #{github_options[:output_dir]}...")
      case System.cmd("/bin/bash", ["-c", "git clone #{github_options[:repo_url]} #{github_options[:output_dir]}"], [{:stderr_to_stdout, true}]) do
        {message, 0} ->
  ...
  def checkout(github) do
    ...
    if (github_options == nil) do
      ...
      case System.cmd("/bin/bash", ["-c", "git checkout #{github_options[:branch]}"], [{:cd, "#{github_options[:output_dir]}"}, {:stderr_to_stdout, true}]) do
        {message, 0} ->
  ...
  def add(github, filepath) do
    ...
    if (github_options == nil) do
      ...
      case System.cmd("/bin/bash", ["-c", "git add #{filepath}"], [{:cd, "#{github_options[:output_dir]}"}, {:stderr_to_stdout, true}]) do
        {message, 0} ->
  ...
  def add_all(github, dir) do
    ...
    if (github_options == nil) do
      ...
      Logger.info("Staging file #{github_options[:output_dir]} for commit...")
      case System.cmd("/bin/bash", ["-c", "git add -A #{dir}"], [{:cd, "#{github_options[:output_dir]}"}, {:stderr_to_stdout, true}]) do
        {message, 0} ->
  ...
  def commit(github, message) do
    ...
    if (github_options == nil) do
      ...
      case System.cmd("/bin/bash", ["-c", "git commit -m \"#{message}\""], [{:cd, "#{github_options[:output_dir]}"}, {:stderr_to_stdout, true}]) do
        {message, 0} ->
  ...
  def push(github) do
    ...
    if (github_options == nil) do
      ...
      case System.cmd("/bin/bash", ["-c", "git push"], [{:cd, "#{github_options[:output_dir]}"}, {:stderr_to_stdout, true}]) do
        {message, 0} ->
