defmodule OpenAperture.Deployer.SystemdUnit do
  @moduledoc """
  Contains the logic for manipulating systemd units
  """
  require Logger
  ...
  def refresh(unit, etcd_token \\ nil) do
    ...
    resolved_etcd_token = resolve_etcd_token(unit, etcd_token)

    Logger.debug("Refreshing unit #{unit_options["name"]} on cluster #{resolved_etcd_token}...")
  ...
  def is_active?(unit, etcd_token \\ nil) do
    unit_options = Agent.get(unit, fn options -> options end)
    current_unit_states = FleetApi.UnitState.list!(resolve_etcd_token(unit, etcd_token))
  ...
  def spinup_unit(unit, etcd_token \\ nil) do
    ...
    case FleetApi.Unit.set_unit(resolve_etcd_token(unit, etcd_token), unit_options, [{"Content-Type", "application/json"}]) do
      %FleetApi.Response{status: 201} ->
        Logger.debug ("Successfully loaded unit #{unit_options["name"]}")
        true
      %FleetApi.Response{status: 204} ->
        Logger.debug ("Successfully loaded unit #{unit_options["name"]}")
        true
      %FleetApi.Response{status: status, body: body} ->
        Logger.error ("Failed to created unit #{unit_options["name"]} (#{status}):  #{inspect body}")
        false
    end
  end
  ...
  def teardown_unit(unit, etcd_token \\ nil) do
    ...
    case FleetApi.Unit.delete_unit(resolve_etcd_token(unit, etcd_token), unit_options["name"]) do
      %FleetApi.Response{status: 201} ->
      ...
      %FleetApi.Response{status: 204} ->
        ...
        wait_for_unit_teardown(unit, etcd_token)
      %FleetApi.Response{status: status, body: body} ->
        Logger.error ("Failed to deleted unit #{unit_options["name"]} (#{status}):  #{inspect body}")
    end
  end
  ...
  defp wait_for_unit_teardown(unit, etcd_token) do
    ...
    resolved_etcd_token = resolve_etcd_token(unit, etcd_token)
    case FleetApi.Unit.get_unit(resolved_etcd_token, unit_options["name"]) do
  ...
  # String
  #
  @spec resolve_etcd_token(pid, String.t()) :: String.t()
  defp resolve_etcd_token(unit, etcd_token \\ nil) do
    if (etcd_token != nil) do
      etcd_token
    else
      unit_options = Agent.get(unit, fn options -> options end)
      unit_options[:etcd_token]
    end
  end
  ...
  def get_journal(unit, etcd_token \\ nil) do
    ...
    if (unit_options["machineID"] != nil) do
      ...
      cluster_hosts = FleetApi.Machine.list!(resolved_etcd_token)
    ...
    case result do
      ...
      _ ->
        Logger.debug("Unable to retrieve logs using the unit's machineID (#{inspect requested_host}), defaulting to all hosts in cluster #{resolved_etcd_token}...")
        hosts = FleetApi.Machine.list!(resolved_etcd_token)
  ...
  def execute_journal_request([requested_host|remaining_hosts], unit_options, verify_result) do
    File.mkdir_p("/tmp/openaperture/systemd_unit")
    stdout_file = "/tmp/openaperture/systemd_unit/#{UUID.uuid1()}.log"
    stderr_file = "/tmp/openaperture/systemd_unit/#{UUID.uuid1()}.log"
    ...
    journal_script_file = "/tmp/openaperture/systemd_unit/#{UUID.uuid1()}.sh"
