defmodule OpenAperture.Deployer.EtcdCluster do
  @moduledoc "Manages an etcd cluster."
  require    Logger
  alias      OpenAperture.Deployer.SystemdUnit

  @doc """
  Starts the GenServer for managing an etcd cluster.
  ...
  @doc """
  Generates a new EtcdCluster agent.
  Returns pid
  """
  ...
  def create!(etcd_token) do
    case create(etcd_token) do
      {:ok, cluster} -> cluster
      {:error, reason} -> raise "Failed to create OpenApertureBuildServer.Agents.EtcdCluster:  #{reason}"
    end
  end
  ...
  def get_hosts(cluster) do
    ...
    try do
      try do
        FleetApi.Machine.list!(cluster_options[:etcd_token])
      rescue _ in FunctionClauseError ->
        Logger.error("Unable to retrieve any hosts in cluster #{cluster_options[:etcd_token]}!")
        []
      end
    rescue _ in RuntimeError ->
      Logger.error("Unable to retrieve hosts - there are no nodes available in cluster #{cluster_options[:etcd_token]}!")
      []
    end
  end
  ...
  def get_units(cluster) do
    ...
    try do
      try do
        FleetApi.Unit.list!(cluster_options[:etcd_token])
      rescue _ in FunctionClauseError ->
        Logger.error("Unable to retrieve any hosts in cluster #{cluster_options[:etcd_token]}!")
        nil
      end
  ...
  def get_units_state(cluster) do
    ...
    try do
      try do
        FleetApi.UnitState.list!(cluster_options[:etcd_token])
      rescue _ in FunctionClauseError ->
        Logger.error("Unable to retrieve any unit state in cluster #{cluster_options[:etcd_token]}!")
        []
      end
    rescue _ in RuntimeError ->
      Logger.error("Unable to retrieve any unit state - there are no nodes available in cluster #{cluster_options[:etcd_token]}!")
      []
    end
  end
  ...
  def get_host_count(cluster) do
    ...
    if hosts, do: length(hosts), else: 0
  end
  ...
  def deploy_units(cluster, new_units, available_ports \\ nil) do
    ...
    existing_units = FleetApi.Unit.list!(cluster_options[:etcd_token])
    ...
    cycle_units(new_units, instance_cnt, cluster_options[:etcd_token], available_ports, existing_units, [])
  end
